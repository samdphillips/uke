...
(test-case "dataframe-reorder-column"
  (check-equal? (for/list ([s (in-list (dataframe-columns students))])
                  (column-name s))
                '(name age color))
  (define s2 (dataframe-reorder-columns students '(age name color)))
  ...
  ; dataframe-reorder-columns can also drop (maybe this is not good?)
  (define s3 (dataframe-reorder-columns students '(age name)))
